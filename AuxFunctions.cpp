...
void AuxFunctions::plotImage(cv::Mat &image, cv::Mat &originalImage, bool right)
{
	...
	if (right)
	{
		...
		cv::imshow("Final detection", originalImage);
	}
	else
	{
...
void AuxFunctions::classifyAndDraw(cv::Mat &originalImage, cv::Mat &finalImage, std::map<int, cv::Vec4i> contours, std::string imageNumber, bool dumpRois, std::string dumpFolder, bool contestMode)
{
	...
	for (i = contours.begin(); i != contours.end(); ++i)
	{
		...
		putText(finalImage, prediction, cv::Point(minX, maxY + 18), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 165, 255), 2);
		contourNumber++;
		...
		{
			if (!contestMode)
				cv::imshow("Example of unprocessed ROI", currentRoiColored);
	...
	for (j = outputBoxes.begin(); j != outputBoxes.end(); ++j)
	{
		...
		putText(finalImage, prediction, cv::Point(minX, maxY + 18), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(128, 128, 0), 2);
		rectangle(originalImage, cv::Point(minX, minY), cv::Point(maxX, maxY), cv::Scalar(212, 255, 127), 2);
		putText(originalImage, prediction, cv::Point(minX, maxY + 18), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(128, 128, 0), 2);
	}
}
